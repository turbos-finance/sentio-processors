...
pool.bind({
  ...
  .onEventSwapEvent(async (event, ctx) => {
    ...
    ctx.meter.Gauge("Swap_Volume_USD").record(usd_volume!, { pairName, pairFullName })
    ctx.meter.Counter("Swap_Volume_USD_Counter").add(usd_volume!, { pairName, pairFullName })
    if (price_a) {
  ...
  .onTimeInterval(async (self, _, ctx) => {
    ...
    if (self) {
      try {
        ...
        ctx.meter.Gauge("TVL_by_Pool_USD").record(tvl, { pairName, pairFullName })
        ctx.meter.Gauge("TVL_by_Pool_Token_USD").record(tvl_a, { pairName, pairFullName, bridge: coin_a_bridge, coin: coin_a_address, symbol: symbol_a })
        ctx.meter.Gauge("TVL_by_Pool_Token_USD").record(tvl_b, { pairName, pairFullName, bridge: coin_b_bridge, coin: coin_b_address, symbol: symbol_b })
